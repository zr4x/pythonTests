...
class ORMFixture:
    ...
    def convert_groups_to_model(self, groups):
        def convert(group):
            return Group(id=str(group.id), name=group.header, footer=group.footer)
        return list(map(convert, groups))

    def convert_contact_to_model(self, contacts):
        def convert(contact):
            return UserForm(id=str(contact.id), firstname=contact.firstname, lastname=contact.lastname)
